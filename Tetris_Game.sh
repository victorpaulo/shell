...
#   Version: 7.0.9 11/01/2011 BeiJing China [Updated 07/02/2013]        #
#                                                                       # 
#   Algorithm:                                                          #
#                                                                       #
#   [][] [][]   [][]            [][]   [][]          [][]   [][] [][]   #
...
{ 
   pos="\e[${i};${j}H"
   if (( ! box_map$((i-modw))[j/2-toph] )); then
         printf "${pos}  "
   else
         str=box_color$((i-modw))[j/2-toph]
...
map.piece()
{
   ...
   for((j=i,u=6; j>toph+1; u+=2)); do
        eval box_map$((j-modw))[u/2-toph]="\${box_map$((j-5))[u/2-toph]}"
        eval box_color$((j-modw))[u/2-toph]="\${box_color$((j-5))[u/2-toph]}"
        (( u == l )) && { u=modw; ((--j)); }
   done
   for((u=6; u<=l; u+=2)); do
        box_map0[u/2-toph]=0
...
random.piece()
{
   ...
   for((i=0,j=6; i!=count; j+=2)); do
        (( j == wthm )) && { j=modw; ((++i)); }
        (( RANDOM%2 )) && { 
           eval box_map$((k-i))[j/2-toph]=1 
           eval box_color$((k-i))[j/2-toph]=\"${color[RANDOM%${#color[@]}]}\"
        }
   done
   (( count == k )) && count=0
}
...
del.row()
{
   ...
   for((i=0; i!=${#cur_box[@]}; i+=2)); do
        ...
        (( len == 16 )) && run.bomb ${#cur_box[@]} || {
           eval box_map$((x-modw))[y/2-toph]=1
           eval box_color$((x-modw))[y/2-toph]=\"${cur_color}\"
        }
   done
   line=0
...
{  
   ...
   for((i=0,j=0; i!=height; j+=2)); do
        (( box_map$((max[j]+i-modw))[max[j+1]/2-toph] == 1 )) && { echo $((i-1)); return 0; }
        (( max[j]+i == lower )) && { echo ${i}; return 0; }
        (( j+2 == ${#max[@]} )) && { j=-2; ((++i)); }
   done
}
...
move.piece()
{
   ...
   for((i=0; i!=len; i+=2)); do    
        ...
        if (( box_map$((x-modw))[y/2-toph] )); then
              if (( len == 2 )); then
                    for((j=lower; j>x; --j)); do
                         (( ! box_map$((j-modw))[y/2-toph] )) && return 0
                    done
              fi
              return 1
        fi
...
ghost.cross()
{
   ...
   ((j=locus[1]))
   str=box_color$((i-modw))[j/2-toph]
   (( box_map$((i-modw))[j/2-toph] )) && printf "\e[${i};${j}H\e[${!str}${unit}\e[0m\n"
}
...
show.notify()
{
   ...
   printf "\e[38;5;106;1m\e[$((toph+19));${dist}HTetris Game  Version 7.0.9\n"
   printf "\e[$((toph+20));${dist}HYongYe <complex.invoke@gmail.com>\e[$((toph+21));${dist}H11/01/2011 BeiJing China [Updated 07/02/2013]\n"
}
   ...
   -v|--version) echo "Tetris Game  Version 7.0.9 [Updated 07/02/2013]" ;;
